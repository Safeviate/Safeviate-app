...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ templates: Array.isArray(config['quality-gap-analysis-templates']) ? config['quality-gap-analysis-templates'] : [] }, { status: 200 });
  } catch (error) {
    console.error('[quality-gap-analysis-templates] fallback to empty list:', error);
...
export async function POST(request: Request) {
  ...
  const config = await getConfig(tenantId);
  const templates = Array.isArray(config['quality-gap-analysis-templates']) ? (config['quality-gap-analysis-templates'] as Array<{ id: string } & Record<string, unknown>>) : [];
  const nextTemplates = templates.some((t) => t.id === incoming.id)
    ? templates.map((t) => (t.id === incoming.id ? incoming : t))
    : [incoming, ...templates];
  const nextConfig = { ...config, 'quality-gap-analysis-templates': nextTemplates };
