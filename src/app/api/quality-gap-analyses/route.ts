...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error('[quality-gap-analyses] fallback to empty payload:', error);
    await recordSimulationRouteMetric({
      tenantId,
      routeKey: 'quality-gap-analyses.GET',
      reads: 0,
...
export async function POST(request: Request) {
  ...
  if (existingRow && existingRow.tenant_id !== tenantId) {
    return NextResponse.json({ error: 'Gap analysis not found in the current tenant.' }, { status: 404 });
  }
  ...
  await recordSimulationRouteMetric({
    tenantId,
    routeKey: 'quality-gap-analyses.POST',
    reads: 0,
    writes: 1,
    durationMs: Date.now() - startedAt,
  });
