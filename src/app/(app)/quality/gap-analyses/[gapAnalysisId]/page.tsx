...
interface GapAnalysisDetailPageProps {
  params: Promise<{ gapAnalysisId: string }>;
}

export default function GapAnalysisDetailPage({ params }: GapAnalysisDetailPageProps) {
  const resolvedParams = use(params);
  const router = useRouter();
  const { tenantId, userProfile } = useUserProfile();
  const { hasPermission } = usePermissions();
  const { isPageEnabled } = usePageLayout('quality-core');
  const gapAnalysisId = resolvedParams.gapAnalysisId;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const payload = await response.json().catch(() => ({ audits: [], templates: [], caps: [], personnel: [], organizations: [], aircraft: [] }));
        ...
        if (!cancelled && foundAudit) {
            ...
            setCaps((payload.caps as CorrectiveActionPlan[] | undefined)?.filter(c => c.auditId === gapAnalysisId) || []);
        }
        if (!cancelled) {
  ...
  useEffect(() => {
    if (!isLoading && audit && userProfile) {
        ...
        if (!canViewAll && userOrgId && audit.organizationId !== userOrgId) {
            router.push('/quality/gap-analyses');
        }
    }
  ...
  if (!audit || !enrichedAudit) {
    return (
      <div className="max-w-[1100px] mx-auto w-full text-center py-20 px-1">
        <p className="text-muted-foreground mb-4">Gap analysis record not found.</p>
        <BackNavButton href="/quality/gap-analyses" text="Back to Gap Analyses" />
      </div>
    );
  }
        ...
        <CardHeader className="shrink-0 border-b bg-muted/5 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 p-6">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-2xl font-black uppercase truncate">Gap Analysis {audit.auditNumber}: {audit.title}</CardTitle>
            </div>
