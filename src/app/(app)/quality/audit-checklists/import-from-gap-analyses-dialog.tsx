...
export function ImportFromGapAnalysesDialog({ onImport }: ImportFromGapAnalysesDialogProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          console.error('Failed to load gap analyses for audit import', error);
          setSources([]);
        }
      } finally {
        if (!cancelled) {
  ...
  const buildSectionsFromSource = (source: GapAnalysisSource): ChecklistSection[] => {
    ...
    return source.template.sections.flatMap((section) => {
      ...
      return [{
        id: uuidv4(),
        title: `${source.auditNumber} - ${source.title}`,
        items: items.map((item) => ({
          id: `${source.id}-${section.id}-${item.id}-${uuidv4()}`,
          text: `${section.title}: ${item.text}`,
          type: item.type,
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Select one or more existing gap analyses to turn into reusable audit checklist sections.
          </DialogDescription>
        </DialogHeader>
