  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setPrivatePilots(Array.isArray(summary.privatePilots) ? summary.privatePilots : []);
        ...
        if (!cancelled) setError(err instanceof Error ? err : new Error('Failed to load private pilots.'));
      } finally {
